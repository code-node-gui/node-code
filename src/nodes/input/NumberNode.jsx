...
function NumberNode({ data , isConnectable ,list}) {
  ...
  return (
    <div className="  hover:scale-[1.025] duration-150  p-2 shadow-lg rounded-md flex bg-[#00356650] backdrop-blur-sm border border-[#fff5]">
        ...
        <p className='text-[#3a86ff] pr-2'>number</p>
        <input style={{width:2+text.length+"ch"}} value={text} type='number' onChange={(e)=>setText(e.target.value)} className=' min-w-[30px] rounded-sm bg-[#fff3] px-2 outline-none text-white'/>
    </div>
  );
}
